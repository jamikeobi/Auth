 ...
 */
  async login(req, res) {
    try {
        ...
        const isPasswordLeaked = await this.enzoicService.checkPassword(body.password);
        if (isPasswordLeaked) {
            return res.status(403).json({
                message: "Password is compromised",
                errors: "Your password has been found in a data breach. Please choose a new one.",
            });
        }

        // Check if credentials are compromised
        const isCredentialLeaked = await this.enzoicService.checkCredentials(body.email, body.password);
        if (isCredentialLeaked) {
            return res.status(403).json({
                message: "Credentials are compromised",
                errors: "Your email and password combination has been found in a data breach. Please reset your password.",
            });
        }
        ...
        return res.status(200).json({ message: "Login successful", data: authResult });
    } catch (error) {
        return res.status(500).json({
...
async _validate(body) {
    ...
    if (body.password && !passwordPattern.test(body.password)) {
        errors.unshift('Password must be at least 6 characters long, contain one uppercase letter, one number, and one special character');
    }
